...
def generate_doc():
    ...
    users_passwords = []
    entries = users_passwords_text.split('\n\n')  # Dividir as entradas por linhas em branco

    for entry in entries:
        lines = entry.splitlines()
        if len(lines) == 1:  # Se não houver pulo de linha, considerar como usuário e senha na mesma entrada
            user = lines[0].split()[0]  # Primeiro elemento da linha é o usuário
            password = ' '.join(lines[0].split()[1:])  # O restante da linha é a senha
        elif len(lines) >= 2:  # Se houver pulo de linha, processar como usuário e senha em entradas separadas
            user = lines[0].strip()
