...
def atualizar_botoes():
    for i, cliente in enumerate(clientes):
        ...
        botoes[i]["labels"]["nome_projeto"].config(bg=color, font=("Helvetica", 10, "bold"), fg="white")  # Título destacado
...
def exibir_projetos():
    ...
    for i, projeto in enumerate(clientes):
        ...
        labels["nome_projeto"] = tk.Label(frame, text=f"{projeto['nome_projeto']}", bg=color, font=("Helvetica", 10, "bold"), fg="white")
        labels["nome_projeto"].grid(row=0, column=0, columnspan=2, sticky="w")
        ...
        button_frame.grid(row=row, column=0, columnspan=2, pady=5, sticky="e")
