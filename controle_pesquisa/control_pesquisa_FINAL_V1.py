...
# Nome do arquivo principal para armazenar todos os dados dos clientes, sempre no mesmo diretório do programa
CURRENT_DIR = os.path.dirname(os.path.abspath(__file__))
DATA_FILE = os.path.join(CURRENT_DIR, "clientes.json")
PESQUISAS_DIR = os.path.join(CURRENT_DIR, "pesquisas_salvas")
...
def salvar_dados():
    filepath = DATA_FILE  # Sempre salva no arquivo padrão do mesmo diretório
...
# Função para marcar como respondido e registrar data/hora da resposta
def marcar_como_respondido(index):
    data_resposta = datetime.now().strftime("%d/%m/%Y %H:%M")
    clientes[index]["aprovado_suporte"] = True
    clientes[index]["data_respondido"] = data_resposta  # Define a data e hora da resposta
    exibir_projetos()  # Atualiza a interface para refletir o status alterado

# Função para marcar como não respondido e remover a data/hora da resposta
def marcar_como_nao_respondido(index):
    clientes[index]["aprovado_suporte"] = False
    clientes[index]["data_respondido"] = ""  # Remove a data/hora de resposta
    exibir_projetos()  # Atualiza a interface para refletir o status alterado
...
def editar_projeto(index):
    ...
    if not is_editing:
        botoes[index]["entries"] = {}
        botoes[index]["edit_labels"] = {}

        # Edição do título (nome do projeto)
        title_label = botoes[index]["labels"]["nome_projeto"]
        title_label.grid_forget()
        title_entry = tk.Entry(frame, font=("Helvetica", 12, "bold"), fg="blue")
        title_entry.insert(0, projeto["nome_projeto"])
        title_entry.grid(row=0, column=1, sticky="w")
        botoes[index]["entries"]["nome_projeto"] = title_entry

        # Label de indicação para o título
        title_edit_label = tk.Label(frame, text="Editando Nome do Projeto:", font=("Helvetica", 9, "bold"), fg="blue")
        title_edit_label.grid(row=0, column=0, sticky="w")
        botoes[index]["edit_labels"]["nome_projeto"] = title_edit_label

        # Edição dos demais campos, incluindo data_criacao e data_respondido, se disponível
        for key, label in botoes[index]["labels"].items():
            if key not in ["nome_projeto"]:
                label.grid_forget()
                entry = tk.Entry(frame, font=("Helvetica", 9), fg="blue")
                entry.insert(0, projeto.get(key, ""))
                entry.grid(row=botoes[index]["positions"][key], column=1, sticky="w")
                botoes[index]["entries"][key] = entry  
                
                # Label indicativa para mostrar qual campo está sendo editado
                edit_label = tk.Label(frame, text=f"Editando {key.capitalize()}:", font=("Helvetica", 9, "bold"), fg="blue")
                edit_label.grid(row=botoes[index]["positions"][key], column=0, sticky="w")
                botoes[index]["edit_labels"][key] = edit_label
        ...
        # Salva as edições e retorna ao modo de exibição normal
        projeto["nome_projeto"] = botoes[index]["entries"]["nome_projeto"].get()
        botoes[index]["entries"]["nome_projeto"].grid_forget()
        botoes[index]["edit_labels"]["nome_projeto"].grid_forget()
        
        # Atualiza e centraliza o título atualizado
        title_text = projeto["nome_projeto"]
        botoes[index]["labels"]["nome_projeto"].config(text=title_text, fg="white", font=("Helvetica", 12, "bold"), anchor="center")
        botoes[index]["labels"]["nome_projeto"].grid(row=0, column=0, columnspan=3, sticky="nsew")

        # Salvamento dos demais campos editados, incluindo datas
        for key, entry in botoes[index]["entries"].items():
            if key != "nome_projeto":
                projeto[key] = entry.get()
                entry.grid_forget()
                botoes[index]["edit_labels"][key].grid_forget()
                label_text = f"{key.capitalize()}: {projeto[key]}"
                text_color = "white" if projeto["aprovado_suporte"] else "black"
                botoes[index]["labels"][key].config(text=label_text, fg=text_color, anchor="w", justify="left")
                botoes[index]["labels"][key].grid(row=botoes[index]["positions"][key], column=0, sticky="w")
...
def excluir_projeto(index):
    if messagebox.askyesno("Confirmar Exclusão", "Tem certeza de que deseja excluir este projeto?"):
        del clientes[index]  # Remove o projeto da lista 'clientes'
        exibir_projetos()    # Atualiza a interface para refletir a exclusão
...
def adicionar_pesquisa():
    ...
    if analista and n_project and nome_projeto and nota_pesquisa and nome_avaliador and email_avaliador and telefone:
        ...
        clientes.insert(0, novo_projeto)
        exibir_projetos()
...
def exibir_projetos():
    ...
    for i, projeto in enumerate(clientes):
        ...
        labels["data_criacao"] = tk.Label(frame, text=f"Criado em: {projeto['data']} {projeto['hora']}", bg=color, font=font_label, fg="yellow")
        labels["data_criacao"].grid(row=row, column=0, sticky="w")
        positions["data_criacao"] = row
        row += 1

        # Exibir campos adicionais
        for key, value in projeto.items():
            if key not in ["aprovado_suporte", "data", "hora", "nome_projeto", "data_respondido"]:
        ...
        # Exibir data/hora da resposta, se disponível
        if projeto.get("data_respondido"):
            labels["data_respondido"] = tk.Label(frame, text=f"Respondido em: {projeto['data_respondido']}", bg=color, font=font_label, fg="yellow")
            labels["data_respondido"].grid(row=row, column=0, sticky="w")
            positions["data_respondido"] = row
            row += 1
...
# Inicializando o programa com o arquivo JSON no mesmo diretório
