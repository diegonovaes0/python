...
def editar_projeto(index):
    ...
    if not is_editing:
        for key, label in botoes[index]["labels"].items():
            if key != "nome_projeto":
                label.grid_forget()
                botoes[index]["labels"][f"{key}_label"] = tk.Label(frame, text=f"{key.capitalize()}: ", font=("Helvetica", 9, "bold"), fg="blue")
                botoes[index]["labels"][f"{key}_label"].grid(row=botoes[index]["positions"][key], column=0, sticky="w")
        ...
        for key, entry in botoes[index]["entries"].items():
            entry.grid_forget()
            botoes[index]["labels"][f"{key}_label"].grid_forget()  # Remove o label de edição
            label_text = f"{key.capitalize()}: {projeto[key]}"
            botoes[index]["labels"][key].config(text=label_text, fg="white" if projeto["aprovado_suporte"] else "black")
...
def exibir_projetos():
    ...
    for i, projeto in enumerate(clientes):
        ...
        botao_nao_respondido = tk.Button(button_frame, text="Não Respondido", command=lambda idx=i: marcar_como_nao_respondido(idx), bg="red", fg="white")
