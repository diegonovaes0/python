...
def editar_projeto(index):
    ...
    if not is_editing:
        ...
        botoes[index]["edit_labels"] = {}  # Armazena labels de indicação de edição
        for key, label in botoes[index]["labels"].items():
            if key != "nome_projeto":
                ...
                botoes[index]["entries"][key] = entry  # Armazena o campo de entrada para salvamento posterior

                # Adiciona uma label indicativa para mostrar qual campo está em edição
        ...
        for key, entry in botoes[index]["entries"].items():
            ...
            projeto[key] = entry.get()  # Obtém o valor atualizado do campo de entrada
            entry.grid_forget()  # Esconde o campo de entrada

            # Remove a label indicativa de edição
            ...
            # Configura a label com sticky="w" para garantir alinhamento ao canto esquerdo
            botoes[index]["labels"][key].grid(row=botoes[index]["positions"][key], column=0, columnspan=2, sticky="w")
