...
def atualizar_botoes():
    exibir_projetos()  # Chama exibir_projetos para garantir atualização completa
...
def marcar_como_respondido(index):
    ...
    clientes[index]["data_respondido"] = data_resposta  # Armazena a data e hora da resposta
    atualizar_botoes()

# Função para marcar como não respondido, remover data/hora e mudar cor para vermelho
...
def excluir_projeto(index):
    if messagebox.askyesno("Confirmar Exclusão", "Tem certeza de que deseja excluir este projeto?"):
        del clientes[index]
        exibir_projetos()  # Chama exibir_projetos para atualizar toda a lista
...
def adicionar_pesquisa():
    ...
    if analista and n_project and nome_projeto and nota_pesquisa and nome_avaliador and email_avaliador and telefone:
        ...
        clientes.insert(0, novo_projeto)  # Insere o novo projeto na primeira posição da lista
        exibir_projetos()
