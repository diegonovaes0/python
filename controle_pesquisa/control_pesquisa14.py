...
def editar_projeto(index):
    ...
    if not is_editing:
        # Inicializa os dicionários para entradas e labels de edição
        ...
        # Salvar edições para o título e demais campos
        
        # Salvamento do título (nome do projeto)
        ...
        # Atualiza a label do título com o novo valor e centraliza
        ...
        for key, entry in botoes[index]["entries"].items():
            if key != "nome_projeto":  # Já processamos nome_projeto acima
                projeto[key] = entry.get()
                entry.grid_forget()
                if key in botoes[index]["edit_labels"]:
                    botoes[index]["edit_labels"][key].grid_forget()

                # Atualiza cada label com o novo valor no modo de visualização, retornando ao início
...
root.geometry("880x600")  # Ajuste da janela para mostrar tudo
