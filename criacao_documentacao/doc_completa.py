...
def display_document_content():
    ...
    def export_to_docx():
        ...
        if os.path.exists(img_path):
            doc.add_picture(img_path, width=Inches(6.3))  # Largura ajustada para preencher a página
    ...
    def export_to_pdf():
        ...
        if os.path.exists(img_path_footer):
            ...
            img_path = os.path.join("img", "capa_doc_autosky.png")
            if os.path.exists(img_path):
               # Adicionar imagem de capa para ocupar a página inteira
                img = Image.open(img_path)
                img = img.resize((int(width), int(height)), Image.LANCZOS)  # Ajuste o tamanho para a página A4

                img_io = BytesIO()
                img.save(img_io, format="PNG")
                img_reader = ImageReader(img_io)

                # Centralizar e preencher a imagem na página
                pdf_canvas.drawImage(img_reader, 0, 0, width=width, height=height)
                y -= height  # Atualiza y para a próxima seção de conteúdo
